#!/usr/bin/env python3
"""
Attach Inspire RH56DFTP-2L LEFT hand to Unitree G1 MuJoCo model.

Source hand model:
    EuncheolIm/Inspire_hand_description
    Inspire_hand/xml/inspire_hand_left.xml

What this script does:
1. Load the original G1 g1_29dof.xml.
2. Load the RH56DFTP-2L left-hand MJCF directly.
3. Convert all hand mesh file paths to absolute paths.
4. Remove the stock G1 left_rubber_hand visual geom.
5. Restore left_wrist_yaw_link to wrist-only inertia.
6. Remove the standalone-scene offset from left_hand_root:
       pos="0 -0.1 0" -> pos="0 0 0"
7. Merge hand <asset>, <equality>, <actuator>, and hand body tree into G1.
8. Mount the hand below left_wrist_yaw_link through a tunable fixed body.
...
# ============================================================
# G1 left wrist-only inertia
#
# The stock g1_29dof.xml may fold the fixed rubber-hand mass
# into left_wrist_yaw_link.  When replacing that hand with the
...
# ============================================================
# Expected RH56DFTP-2L left hand interface
# ============================================================

EXPECTED_HAND_ACTUATORS = [
    "left_thumb_yaw_act",
    "left_thumb_pitch_act",
    "left_index_act",
    "left_middle_act",
    "left_ring_act",
    "left_pinky_act",
...
def remove_stock_left_rubber_hand(wrist_body):
    """
    ...
    for child in list(wrist_body):
        ...
        if (
            mesh_name == "left_rubber_hand"
            or geom_name == "left_rubber_hand"
        ):
...
def restore_wrist_only_inertial(wrist_body):
    """
    Restore left_wrist_yaw_link's inertial parameters so the
    ...
    inertial.set(
        "pos",
        LEFT_WRIST_ONLY_INERTIAL["pos"],
    )

    inertial.set(
        "mass",
        LEFT_WRIST_ONLY_INERTIAL["mass"],
    )

    inertial.set(
        "fullinertia",
        LEFT_WRIST_ONLY_INERTIAL["fullinertia"],
    )
...
def get_single_hand_root_body(hand_root):
    """
    RH56DFTP left-hand MJCF contains one top-level hand body:
        left_hand_root
    ...
    for body in bodies:
        if body.get("name") == "left_hand_root":
            target = body
            break
    ...
    if target is None:
        if len(bodies) == 1:
            ...
            raise RuntimeError(
                "Could not uniquely identify RH56DFTP left hand root. "
...
def validate_source_hand_model(hand_xml_path):
    """
    Compile the original hand MJCF before modifying anything and
    verify its expected 12 joints / 6 actuators / 6 equalities.
    """
    ...
    print(
        "Source RH56DFTP left-hand model"
    )
    print(
        "=========================================="
    )
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Attach RH56DFTP-2L Inspire LEFT hand MJCF "
            "to Unitree G1 left_wrist_yaw_link."
        )
    )
    ...
    parser.add_argument(
        ...
        help="RH56DFTP-2L LEFT hand MuJoCo XML.",
    )
    ...
    #
    # G1 left wrist distal direction is +X.
    #
    # Ry(+pi/2):
    #     +Z_hand -> +X_G1
    #
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "left_wrist_yaw_link coordinates."
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Keep the stock G1 left_rubber_hand geom. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Keep left_hand_root's source pos='0 -0.1 0'. "
    ...
    if not args.keep_rubber_hand:
        removed = remove_stock_left_rubber_hand(
            wrist_body
        )

        print(
            "Removed stock left_rubber_hand geoms:",
            removed,
        )

    else:
        print(
            "Keeping stock left_rubber_hand geom."
        )
    ...
    if not args.keep_combined_wrist_inertia:
        ...
        print(
            "Restored left_wrist_yaw_link wrist-only inertia."
        )
    ...
    print()
    print(
        "RH56DFTP source left_hand_root pos:",
        source_root_pos,
    )
    print(
        "RH56DFTP attached left_hand_root pos:",
        hand_body.get(
            "pos",
            "0 0 0",
        ),
    ...
    if mount_id < 0:
        raise RuntimeError(
            "left_rh56dftp_mount is missing after compilation."
        )

    if hand_root_id < 0:
        raise RuntimeError(
            "left_hand_root is missing after compilation."
        )

    print()
    print(
        "PASS: G1 + RH56DFTP left-hand model compiles successfully."
    )
    print()
    print(
        "Next step:"
